...
把本地图片上传到指定演示文稿的 drive 媒体库，返回 `file_token`。**返回的 token 作为 `<img src="...">` 的值塞进 slide
XML 即可显示图片。**
...
> [!IMPORTANT] **路径必须在 CWD 内**：`--file /abs/path/x.png` 或 `--file ../up/x.png` 会被 CLI 拒绝（报
...
拿到 `file_token` 后走 [`+replace-slide`](lark-slides-replace-slide.md) 的 `block_insert`，不用搬原 XML、不改
`slide_id`、不打乱页序：
...
2. **`<img>` 的 `width:height` 对齐原图比例** —— 比例不一致会被裁剪，参见
   [xml-schema-quick-ref.md](xml-schema-quick-ref.md) `<img>` 说明
...
- [xml_presentation.slide create](lark-slides-xml-presentation-slide-create.md)
  — 创建 slide 页面（拿到 file_token 后塞进 XML）
