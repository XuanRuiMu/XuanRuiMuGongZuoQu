...
- For Codex Desktop 26.707, build the native replacement with `scripts\build-remote-control-native-replacement.ps1 -WorkRoot "<large-local-build-root>\native-remote-0.144.0-alpha.4" -CodexSourceRef "rust-v0.144.0-alpha.4" -AppServerVersion "0.144.0-alpha.4"`. Historical `0.142.4` has a separate patch-apply-validated patch but is not yet fully compilation-validated; any other version needs exact matching parameters plus a validated `-PatchPathOverride`.
