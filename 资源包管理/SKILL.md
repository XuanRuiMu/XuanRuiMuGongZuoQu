...
name: "资源包管理"
description:
  'Minecraft Java版资源包制作与部署。涵盖1.21.4+新版Item Model系统和传统CustomModelData系统，以及暮澜纪元资源包自动压缩打包与部署到客户端。
  触发场景：制作资源包、创建材质包、插件自定义物品纹理、item_model、CustomModelData、更新资源包、打包资源包、部署资源包。
  当用户说"制作资源包"、"创建材质包"、"自定义物品纹理"、"item_model"、"CustomModelData"、"资源包不生效"、"材质包不显示"、"更新资源包"、"打包资源包"、"部署资源包"、"压缩资源包"、"材质包更新"、"材质包部署"时触发此技能。'
---

# Minecraft Java版资源包管理技能
...
检查源目录和 `pack.mcmeta` 是否存在，不存在则停止。

#### 第2步：压缩资源包

使用 PowerShell 压缩，命名格式：`暮澜纪元资源包_v{版本}_{日期}.zip`
...
**关键**：使用 `源目录\*` 而非 `源目录`，确保 zip 顶层是 `pack.mcmeta` 和 `assets/`。
...
扫描目标目录，删除以 `暮澜纪元资源包` 开头的旧 zip 文件，排除刚部署的文件。

#### 第5步：报告

用 AskUserQuestion 报告：新文件名、大小、删除的旧文件、释放空间、部署路径。
