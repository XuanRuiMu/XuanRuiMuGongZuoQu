...
关闭收到邮件的已读回执请求 banner，**但不向发件人发送回执**。**本命令仅在对方邮件请求了已读回执（`READ_RECEIPT_REQUEST`
标签，系统 ID `-607`）时使用**。对齐飞书客户端上已读回执 banner 右侧的"不发送"按钮。
...
- 先 `fetchFullMessage` 拉一遍原邮件校验：若 `label_ids` 中不含 `READ_RECEIPT_REQUEST`（也不含数字 `-607`），直接返回
  `already_cleared: true`，**不发请求**，幂等。
- 标签存在时调 `PUT /user_mailboxes/<mailbox>/messages/<id>/modify`（`user_mailbox.message.modify`），body
