...
更新已有的个人邮件模板（全量替换式更新）。支持 `--inspect` 只读 projection、`--print-patch-template`
打印 patch 骨架、`--patch-file` 结构化 patch、以及扁平的 `--set-*` flag。

> **⚠️ 后端无乐观锁 →
> last-write-wins**。并发更新可能丢失最近的改动；CLI 在每次成功更新时会在 stderr 打印一条 warning 提示。
...
> **所有原有附件保留**：只追加 `--attach` 新附件；如需删除已有附件，目前只能通过 `--patch-file` 的 `template_content`
> 改写正文去除相应 `<img>` 引用，或使用原生 API 整块重写。
