...
p, h1-h9, ul, ol, li, table, thead, tbody, tr, th, td, blockquote, pre, code, hr, img, b, em, u, del, a, br,
span 语义不变
...
- `<whiteboard>` — `<whiteboard type="blank"></whiteboard>`
  空白；`<whiteboard type="mermaid|plantuml">内容</whiteboard>` 带内容；
...
支持**所有**块类型（块级标签、容器标签、行内组件、资源块），使用
`docs +update --command block_move_after --block-id "<锚点>" --src-block-ids "id1,id2"`。
...
- 颜色优先使用命名色，也可写 `rgb(r,g,b)` / `rgba(r,g,b,a)`。**基础色（7 色）**：red, orange, yellow, green, blue,
  purple, gray
