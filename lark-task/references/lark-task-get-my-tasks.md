...
If the user query only specifies a task name (e.g., "Complete task Lobster No. 1"), use this command to list and search
for the task by its summary.
...
>
> 1. If you need to present user fields (assignee, creator, etc.), do not only output the raw `id` (e.g. open_id). Also
>    try to resolve and display the user's real name (e.g. via the contact skill) for readability.
...
2. Execute the command. The CLI will automatically loop up to the specified limit (default 20, or 40 with `--page-all`)
   to fetch records.
