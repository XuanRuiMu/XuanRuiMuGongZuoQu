...
> [!IMPORTANT] **用户配色优先。**
> 用户指定了色值/风格时以用户为准。用户只给 1-2 个色值时，推导完整色板：主色→浅底→深边框→灰调连线色。用户**未指定**配色时，必须从上方色板表中选取颜色，不要使用表中没有的自创色值（如
> `#E8F3FF`、`#1664FF`、`#14C9C9` 等都不在色板中）。
...
**所有分区标签统一用深色文字
`#1F2329`**，不要给每个标签用不同颜色——颜色区分通过层容器背景和边框体现，标签文字颜色保持一致。
