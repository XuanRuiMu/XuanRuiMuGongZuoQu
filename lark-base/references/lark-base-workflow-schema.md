...
> [workflow-guide.md](lark-base-workflow-guide.md)。
...
> ⚠️ **所有涉及用户的 value 中的 id 统一使用 OpenID（`ou_xxxx` 格式）**，由 CLI 层来完成转换 ⚠️
> **所有涉及群的 value 中的 id 统一使用 ChatID（`oc_xxxx` 格式）**，由 CLI 层来完成转换

### ref 引用变量详解

`ref` 类型是工作流中节点间数据传递的核心机制。当 `value_type` 为 `ref` 时，`value`
指向前置节点的某个输出变量。本节详细描述每个节点可供引用的输出变量定义。
...
**recordLink 的 children**：如果配置了数据表，则为该表所有视图的列表，每个视图
`{ pathId: viewId, pathName: viewName, pathType: 'string' }`。引用示例：`$.{stepId}.recordLink.{viewId}`。
...
**场景一：遍历组合记录** — 数据源为 `record` 类型时（如 FindRecordAction 的 `fieldRecords`），`item` 类型为
`record`，可向下选择具体字段：
...
每个字段变量都可以进一步下钻选择字段的属性。所有字段至少支持 `fieldId` 和 `fieldName`
两个基础属性，部分字段还支持额外属性：
...
> 其他字段类型（如 `text`、`number`、`checkbox`、`select`、`location`、`formula`、`lookup` 等）仅支持 `fieldId` 和
> `fieldName` 两个基础属性。
...
{
  "conjunction": "and",
  "conditions": [{ "field_name": "状态", "operator": "is", "value": [{ "value_type": "text", "value": "进行中" }] }]
}
```
...
**operator 可选值：** `is` / `isNot` / `containsAny` / `doesNotContainAny` / /`containsAll`/ `isEmpty` / `isNotEmpty` /
...
> ⚠️ **Select 字段条件**：`value_type` 必须为 `option`，`value` 对象可只传 `name`（如
> `{"name": "L2"}`），无需提供选项 ID。
  ...
  "steps": [
    ...
    },
    {
      ...
      "data": {
        ...
        "field_values": [{ "field_name": "审批状态", "value": [{ "value_type": "text", "value": "已通过" }] }]
      }
    },
    {
      "id": "step_5",
