...
> [!CAUTION] 这是写入操作。用户已经明确要上传到某条记录的某个附件字段时可直接执行；如果 `record-id`
> 或目标字段仍有歧义，再先确认。

## 坑点

- ⚠️ 目标字段必须是 `attachment` 字段。
- ⚠️ 记录里的附件 `file_token` 属于 Drive media token；下载时不要走 `lark-cli drive +download`，应使用
  `lark-cli docs +media-download --token <file_token> --output <path>`。
