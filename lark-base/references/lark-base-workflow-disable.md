...
> ⚠️ **注意区分 ID 前缀**：table_id 以 `tbl` 开头，workflow_id 以 `wkf` 开头，两者在 URL 的 `?table=`
> 参数里都会出现，需要根据前缀判断。
...
- ⚠️ **workflow_id 来源**：workflow_id 以 `wkf` 开头，从 URL 的 `?table=wkf...` 参数提取，不是表格的 table_id（`tbl`
  开头），混淆会导致 `[2200] Internal Error`
...
- ⚠️ **scope 待确认**：内部文档未列出权限名称，代码中使用 `base:workflow:update`，如遇 `[230013] permission denied`
  需核对实际 scope
