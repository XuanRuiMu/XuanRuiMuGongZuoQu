...
>
> **前置条件：** 需要base开启了高级权限，如果调用这个接口返回没有开启高级权限，可以参考
> [`lark-base-advperm-enable.md`](lark-base-advperm-enable.md) 了解高级权限启用规则。
...
- ⚠️ **Filter 字段**：`field_name`、`operator`、`filter_values`
  由客户端传入，`field_type`、`field_ui_type`、`reference_type` 由服务端 filterFiller 自动补全
