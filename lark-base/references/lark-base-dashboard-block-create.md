...
> **前置条件：** 先阅读 [lark-base-dashboard.md](lark-base-dashboard.md) 了解整体工作流 **关键：** 创建前必须阅读
> [dashboard-block-data-config.md](dashboard-block-data-config.md) 了解组件类型和 data_config 结构
...
- **`data_config` 结构随 `type` 变化**，不同组件类型字段不同，**⚠️ 必须阅读
  [dashboard-block-data-config.md](dashboard-block-data-config.md) 了解如何构造**
- **组件创建必须串行执行**，不能并发
